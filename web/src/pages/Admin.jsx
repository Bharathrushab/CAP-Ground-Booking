...
function AnnouncementImageField({ record, update, preview }) {
  ...
  async function upload(event) {
    ...
    event.target.value = '';
    if (!file) return;
    if (!record.id) { setError('Enter a title and URL slug before uploading.'); return; }
    ...
    try {
      ...
      const object = storageRef(storage, `announcement-images/${record.id}/${Date.now()}.${extension}`);
      await uploadBytes(object, file, { contentType: file.type });
      update('imageUrl', await getDownloadURL(object));
      if (!record.imageAlt) update('imageAlt', record.title);
    } catch (failure) { setError(failure.message); } finally { setUploading(false); }
  }
  return <fieldset className="image-field"><legend>Announcement picture</legend>{record.imageUrl && <img src={record.imageUrl} alt={record.imageAlt || ''} />}{preview ? <p className="muted">Image upload is available on the production CMS.</p> : <label className="button small outline image-upload"><Upload size={16} /> {uploading ? 'Uploading...' : record.imageUrl ? 'Replace picture' : 'Upload picture'}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={uploading} onChange={upload} /></label>}{record.imageUrl && <button type="button" className="text-link danger" onClick={() => { update('imageUrl', ''); update('imageAlt', ''); }}>Remove picture</button>}<Field label="Image description" value={record.imageAlt} maxLength={160} disabled={!record.imageUrl} onChange={(value) => update('imageAlt', value)} />{error && <p className="alert error" role="alert">{error}</p>}<small>JPEG, PNG, WebP or GIF. Maximum 5 MB.</small></fieldset>;
}
